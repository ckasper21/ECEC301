    ...
    def _loadDeck(self):
        ...
        while count<37:
            for j in range(2,11):
                for k in range(4):
                    if k==0:
                        self.pushCard(Card(j,"clubs"))
                        count+=1
                    if k==1:
                        self.pushCard((Card(j,"spades")))
                        count+=1
                    if k==2:
                        self.pushCard((Card(j,"hearts")))
                        count+=1
                    if k==3:
                        self.pushCard(Card(j,"diamonds"))
                        count+=1
        #Generate Face Cards
        while count<49:
            for j in range(11,14):
                for k in range(4):
                    if k == 0:
                        self.pushCard(FaceCard(j, "clubs"))
                        count +=1
                    if k == 1:
                        self.pushCard(FaceCard(j, "spades"))
                        count +=1
                    if k == 2:
                        self.pushCard(FaceCard(j, "hearts"))
                        count +=1
                    if k == 3:
                        self.pushCard(FaceCard(j, "diamonds"))
                        count +=1
        #Generate Aces
        while count<53:
            for k in range(4):
                if k == 0:
                    self.pushCard(AceCard("clubs"))
                    count +=1
                if k == 1:
                    self.pushCard(AceCard("spades"))
                    count +=1
                if k == 2:
                    self.pushCard(AceCard("hearts"))
                    count +=1
                if k == 3:
                    self.pushCard(AceCard("diamonds"))
                    count +=1
...
class Hand(CardContainer):
    ...
    def getSoftScore(self):
        ...
        """
        ###################################################################
        soft_score=self.getHoleCard().softValue()
        for card in self:
            soft_score=soft_score+card.softValue()
        return soft_score
        ###################################################################
    ...
    def getHardScore(self):
        ...
        """
        ###################################################################
        hard_score=self.getHoleCard().hardValue()
        for card in self:
            hard_score=hard_score+card.hardValue()
        return hard_score
        ###################################################################
...
class Game(object):
    ...
    def _doHouseAi(self):
        ...
        """
        ###################################################################
        if (self._house.getHardScore()!=21) and (self._house.getSoftScore()!=21):
            while ((self._house.getSoftScore() < 18) or (self._house.getSoftScore() > 21)) and (self._house.getHardScore() < 18):
    ...
    def _checkWhoWins(self):
        ...
        """
        ###################################################################
        ps=self._player.getSoftScore()
        ph=self._player.getHardScore()
        hs=self._house.getSoftScore()
        hh=self._house.getHardScore()

        if self._checkPlayerBust():
            print "House Wins!"
        elif self._checkHouseBust() or ((ps==21) or (ph==21)):
            print "Player Wins!"
        elif ps > 21:
            if hs > 21:
                if ph > hh:
            ...
            if hs > 21:
                ...
                if ((ps > hh) and (ps > hs)) or ((ph > hh ) and (ph > hs)):
                    print "Player Wins!"
                elif ((ps==hs) or (ps==hh)):
                    print "Player Wins!"
                else:
